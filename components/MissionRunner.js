...
export default function MissionRunner({
  ...
  async function handleBack() {
    setSaving(true);
    await saveProgress(status, status === "done" ? mission.xp : 0, values);
    setSaving(false);
    router.push(`/journey/${levelId}/${previousMissionId}`);
  }
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-ink/90 px-5">
          <div className="w-full max-w-sm rounded-3xl border-2 border-ink bg-white p-8 shadow-card">
            ...
            <p className="mt-3 font-body text-[14px] leading-relaxed text-inkSoft">
              This clears every mission in {levelTitle} — your answers, progress, and
              XP from it. This can&rsquo;t be undone.
            </p>
            {resetError ? (
